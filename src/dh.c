...
static int make_entropy()
{
    ...
    printf("No random state found, trying to generate entropy from "
	   "/dev/random...\n");
	...
	printf("You can place a file containing random data called "
	       ".ircd.entropy\n");
	printf("in the directory with your ircd.conf. This file must be "
	       "at least %d\n", RAND_BYTES);
...
static int init_random()
{
    ...
    if(!(fp = fopen(".ircd.entropy", "w")) || mpz_sizeinbase(tmp, 16) > 
       RAND_BYTES_HEX)
    {
	if(!fp)
