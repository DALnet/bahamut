...
# if !defined(SOL20) || defined(USE_SYSLOG)
   ...
   if (setsockopt(fd, SOL_SOCKET, SO_DEBUG, (char *) &opt, sizeof(opt)) <
       0) report_error("setsockopt(SO_DEBUG) %s:%s", cptr);
# endif				/* 
				 * SOL20
				 */
#endif
...
# ifdef	HPUX
      nfds =
	  select(FD_SETSIZE, (fd_set *) read_set, (fd_set *) write_set,
		 (fd_set *) 0, &wait);
# else
...
# endif
	       ...
	       if (
		   (fd =
		    accept(i, (struct sockaddr *) &addr, &addrlen)) < 0) {
