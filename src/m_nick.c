...
int m_nick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
   ...
   */
   ...
   if (do_nick_name(nick) == 0 || (IsServer(cptr) && strcmp(nick, parv[1]))) {
	 ...
	 sendto_serv_butone(cptr, ":%s KILL %s :Bad Nick", me.name, parv[0]);
	...
	*/
     ...
     if (IsUnknown(acptr)) {
       if (MyConnect(acptr)) {
         ...
         sendto_serv_butone(NULL, ":%s KILL %s :Nick Collission", me.name,
			    acptr->name);
	...
	*/
     ...
     if (!newts || !acptr->tsinfo || (newts == acptr->tsinfo) || !sptr->user) {
	   ...
	   (!sameuser && newts > acptr->tsinfo)) {
         if (sameuser)
	   sendto_ops_lev(SKILL_LEV, "Nick change collision from %s to %s",
