...
/*
 *  * authsenderr() *  * input         - pointer to aClient output
 *      - NONE side * effects 
 */

static void
authsenderr(aClient *cptr)
{
   ircstp->is_abad++;
   ...
   if ((len > 0) && (cptr->count != sizeof(cptr->buffer) - 1) &&
      ...
      *t = '\0';
      Debug((DEBUG_INFO, "auth reply ok"));
				 ...
				 */
      /*
       * sendto_realops("This is the infamous fdlist.c bug.
       * congrats.");
       */
      *ruser = '\0';
   }
