 ...
 * try_connections 
 * 
 *      Scan through configuration and try new connections. *   Returns
 * the calendar time when the next call to this *       function should
 * be made latest. (No harm done if this *      is called earlier or
 * later...)
 */
static      time_t
 ...
 * 
 * check_pings() inputs - current time output   - next time_t when
 * check_pings() should be called again
 * 
 * side effects -
 * 
 * Clients can be k-lined/z-lined/g-lined/r-lined and exit_client called
 * for each of these.
 * 
 ...
 */
/*
 * Note, that dying_clients and dying_clients_reason really don't need
 * to be any where near as long as MAXCONNECTIONS but I made it this
 * long for now. If its made shorter, then a limit check is going to
 * have to be added as well -Dianora
 */

aClient    *dying_clients[MAXCONNECTIONS];	/*
...
register aClient *cptr;		/*
				 ...
				 * current local cptr being examined 
				 */
aConfItem  *aconf = (aConfItem *) NULL;
int         ping = 0;		/*

				 * ping time value from client 
				 */
int         i;			/*

				 * used to index through fd/cptr's 
				 */
time_t      oldest = 0;		/*
...
int         die_index = 0;	/*
   ...
   dying_clients[0] = (aClient *) NULL;		/*
						 * mark first one empty 
						 */
   /*
    * I re-wrote the way klines are handled. Instead of rescanning the
    * local[] array and calling exit_client() right away, I mark the
    * client thats dying by placing a pointer to its aClient into
    * dying_clients[]. When I have examined all in local[], I then
    * examine the dying_clients[] for aClient's to exit. This saves the
    * rescan on k-lines, also greatly simplifies the code,
    * 
    * Jan 28, 1998 -Dianora
    */
   ...
   for (i = 0; i <= highest_fd; i++) {
	 ...
	 continue;		/*
      ...
      if (cptr->flags & FLAGS_DEADSOCKET) {
	 /*
	  * N.B. EVERY single time dying_clients[] is set it must be
	  * followed by an immediate continue, to prevent this cptr
	  * from being marked again this will cause exit_client() to be
	  * called twice for the same cptr. i.e. bad news -Dianora
	  */
	 ...
	 continue;		/*
      ...
      /*
       * To make the exit reason have the kline reason I have to keep a
       * copy of the reason in dying_clients_reason[] IF
       * KLINE_WITH_REASON is defined. It is going to be slightly
	 ...
	 ping = get_client_ping(cptr);
      ...
      ((!IsRegistered(cptr) && (currenttime - cptr->since) >= ping))) {
	 /*
	  * ok. There has been a ping time out. determine what sort of
	  * ping timeout Can be one of client ping out, server ping
	  * out, or time out on authd or DNS
	  * 
	  * -Dianora
	  */
	 ...
	 if (ping < currenttime - cptr->lasttime) {
	     ...
	     * from it in a while, PING it to make sure it is still
	     * alive.
	     */
	    cptr->flags |= FLAGS_PINGSENT;
	    /*
	 ...
	 if (cptr->firsttime ? ((timeofday - cptr->firsttime) > 100) : 0) {
	    /*
	     * Lets for the time being, make sure this cptr hasn't
	     * already been marked for exit
	     */

	    if (cptr->flags && (FLAGS_RESTRICTED |
				FLAGS_PING_TIMEOUT |
				FLAGS_KILLFLAG |
				FLAGS_GKILLFLAG |
				FLAGS_ZKILLFLAG)) {
	        ...
	        */
	       sendto_ops("cptr already marked for exit! cptr->flags = %x",
			  cptr->flags);
	    }
	    else {
	       dying_clients[die_index++] = cptr;
	       dying_clients[die_index] = (aClient *) NULL;
	       cptr->flags |= FLAGS_CONNECTION_TIMEDOUT;
	    }
	 }
      }
   ...
   /*
    * Now exit clients marked for exit above. it doesn't matter if
    * local[] gets re-arranged now
    * 
    * -Dianora
    */
   ...
   for (die_index = 0; (cptr = dying_clients[die_index]); die_index++) {
	 ...
	 (void) exit_client(cptr, cptr, &me, (cptr->flags & FLAGS_SENDQEX) ?
      ...
      else if (cptr->flags & FLAGS_KILLFLAG) {
#ifdef KLINE_WITH_REASON
	 ...
	 (void) exit_client(cptr, cptr, &me, "you have been K-lined");
      ...
      else if (cptr->flags & FLAGS_ZKILLFLAG) {
#ifdef KLINE_WITH_REASON
	 (void) exit_client(cptr, cptr, &me, dying_clients_reason[die_index]);
#else
	 (void) exit_client(cptr, cptr, &me, "you have been Z-lined");
      ...
      else if (cptr->flags & FLAGS_GKILLFLAG) {
#ifdef KLINE_WITH_REASON
	 (void) exit_client(cptr, cptr, &me, dying_clients_reason[die_index]);
#else
	 (void) exit_client(cptr, cptr, &me, "you have been G-lined");
      ...
      else if (cptr->flags & FLAGS_PING_TIMEOUT) {
char        ping_time_out_buffer[64];
	 ...
	 (void) exit_client(cptr, cptr, &me, ping_time_out_buffer);
      ...
      else if (cptr->flags & FLAGS_CONNECTION_TIMEDOUT) {
	 (void) exit_client(cptr, cptr, &me, "Connection Timed Out");
      }
   }
   ...
   return (oldest);
        ...
        */
/*
 * * bad_command *    This is called when the commandline is not
 * acceptable. *        Give error message and exit without starting
 * anything.
 */
static int
bad_command()
...
#ifdef INITIAL_DBUFS
   dbuf_init();			/*
								 * set up some dbuf stuff to control
								 * * paging 
								 */
#endif
...
normal user.\n");
   ...
   clear_scache_hash_table();	/*
										 * server cache name table 
										 */
   clear_ip_hash_table();	/*
									 * client host ip hash table 
									 */
   initlists();
   initclass();
   initwhowas();
...
time_t      lasttimeofday;
    ...
    * might think something more clever here... --msa) * We don't
    * really need to check that often and as long * as we don't delay
    * too long, everything should be ok. * waiting too long can cause
    * things to timeout... * i.e. PINGS -> a disconnection :( * -
    * avalon
    */
   if (delay < 1)
      delay = 1;
...
register int i, j;
   ...
   busycli_fdlist.last_entry = j;	/*
					 * rest of the fdlist is
					 * * garbage 
					 */
