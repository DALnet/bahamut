...
do_msg_tree(MESSAGE_TREE * mtree, char *prefix,
	    struct Message *mptr)
{
   char        newpref[64];	/*

				 * must be longer than any command 
				 */
   int         c, c2, lp;
   MESSAGE_TREE *mtree1;
	       ...
	       char *buffer)
{
   ...
   if (!strchr(sender, '.'))
      ...
      sendto_realops_lev(DEBUG_LEV, "Unknown prefix (%s) from %s, Squitting %s", buffer, get_client_name(cptr, HIDEME), sender);
