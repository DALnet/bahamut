...
static struct pollfd devpoll_fds[MAXCONNECTIONS];

void engine_init()
{
    devpoll_id = open("/dev/poll", O_RDWR);
    memset(devpoll_fds, 0, sizeof(devpoll_fds));
}
...
void engine_add_fd(int fd)
{
    ...
    set_fd_internal(fd, (void*)dev_fd);
    ++numfds;
}

void engine_del_fd(int fd)
{
    struct pollfd *dev_fd = (struct pollfd*)get_fd_internal(fd);
    ...
    if (dev_fd != &devpoll_fds[numfds-1])
    {
        *dev_fd = devpoll_fds[numfds-1];
        set_fd_internal(dev_fd->fd, dev_fd);
    }
    --numfds;
}
...
int engine_read_message(time_t delay)
{
    ...
    do
    {
        ...
        {
            dev_fd = (struct pollfd*)get_fd_internal(pevent->fd);
            if (dev_fd->fd != -1)
            {
                int rr = (dev_fd->events & (POLLIN|POLLHUP|POLLERR));
                int rw = (dev_fd->events & POLLOUT);
                
                get_fd_info(dev_fd->fd, &fdtype, &fdflags, &fdvalue);
