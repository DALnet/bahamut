   ...
   * parc == 11 on a normal TS style server-to-server NICK introduction
	...
	*/
       sendto_ops_lev(SKILL_LEV, "Nick collision on %s",
		      sptr->name);
       ircstp->is_kill++;
       sendto_one(cptr, ":%s KILL %s :Nick Collission", me.name, sptr->name);
       sptr->flags |= FLAGS_KILLED;
     ...
     if (IsUnknown(acptr))
     {
       ...
       else if (!(acptr->user))
       {
         ...
         sendto_serv_butone(NULL,
                ":%s KILL %s :Nick Collission on %s", me.name,
                acptr->name, parv[1]);
	...
	*/
	   ...
	   || (newts == acptr->tsinfo))
       {
         ...
         sendto_serv_butone(NULL, ":%s KILL %s :Nick Collission :%s",
     ...
     {
       sendto_ops_lev(SKILL_LEV,
		      "Nick change collision from %s to %s(%s <- %s)",
