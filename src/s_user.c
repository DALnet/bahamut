...
int m_nick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
   ...
   * parc == 11 on a normal TS style server-to-server NICK introduction
    */
   if (((IsServer(sptr) && !IsNICKIP(sptr)) || (parc > 4)) && (parc < 10))
   {
     /*
	...
	*/
       sendto_ops_lev(SKILL_LEV, "Nick collision on %s",
		      sptr->name);
       ircstp->is_kill++;
       sendto_one(cptr, ":%s KILL %s :Nick Collision", me.name, sptr->name);
       sptr->flags |= FLAGS_KILLED;
	...
	*/
     ...
     {
       sendto_ops_lev(SKILL_LEV,
		      "Nick change collision: %s", parv[1]);
       ircstp->is_kill++;
       sendto_one(acptr, err_str(ERR_NICKCOLLISION),
		  me.name, acptr->name, acptr->name);
       sendto_serv_butone(NULL,
       ...
       {
	   sendto_ops_lev(SKILL_LEV,
			  "Nick collision on %s", acptr->name);
	...
	* complain loudly and ignore it. - lucas */
	 ...
	 sendto_ops_lev(DEBUG_LEV,
			"Extra Param without NICKIP from %s",sptr);	 
       }
